// THIS WAS THE OLD STATE STORE -- IT'S BEING RETIRED
// STATE NOW LIVES IN TimersContext.js
...
class Pomodoro extends Component {
  ...
  componentDidMount() {
    // check if there is any local storage state
    if (localStorage.state && localStorage.state.length) {
      const state = JSON.parse(localStorage.getItem('state'));
      // set pause to true on load (timer function will have stopped running)
